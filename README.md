...
- **Function Calling**：小音会自己调用工具——查天气/空气质量（Open-Meteo 实时数据）、报时间日期、农历节气节日（本地计算）、算数（AST 白名单安全计算）、汇率换算、新闻快讯（60秒读懂世界）、日程提醒（到点主动提醒）、备忘录（本地持久化）、快递物流（快递鸟，可选 Key）
...
| 🔧 工具调用 | Function Calling | 天气/空气质量（Open-Meteo）/ 时间 / 农历节日 / 计算器 / 汇率 / 新闻快讯 / 日程提醒 / 备忘录 / 快递（可选） |
