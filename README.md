...
按 `c` 可切换**连续对话**（唤醒一次连着问），按 `r` 重置对话，按 `q` 退出。
...
**连续对话**（默认关，点「🔁 连续对话」按钮开启；桌宠右键菜单、CLI 按 `c` 也能切）：

开启后**唤醒一次就能连着问**，回复完直接说下一句即可，不用每句都喊「小音」。停 `CONTINUOUS_TIMEOUT`
秒（默认 10 秒）没说话就自动休眠回待唤醒，客户端会提示「连续对话结束」。

上限与边界（实现上刻意收紧，避免误触发刷屏）：

- 只有「正常回复完」和「被用户打断」两种情况会续听；**没听到声音 / 语音太短 / 识别为空一律退出**
  —— 否则环境噪声会让轮次无限循环
- 连续对话期间麦克风一直开着（这是它的代价），所以做成了开关且默认关
- 聆听/播报期间关键词检测不再喂流（既有行为），所以在连续模式里喊「小音」会被当成普通话语
...
- **每次比对都会把相似度打进日志**：`[声纹] 相似度 0.68（阈值 0.60）→ 通过`。
  老是"主人却被拒"，就看这个数：如果主人自己唱出来是 0.5 上下，把阈值调到比它低一点
  （比如 0.45）即可，不用瞎猜。
- 两个"宁可不拦"的保护：**有效语音不足 0.4 秒**（离得远、只说了一个字）和提取失败时**直接放行** ——
  拒绝一次要等 5 秒冷却，把主人挡在门外的代价比漏认一次大得多
- **自适应**：相似度 ≥0.75 的高分通过会把这次并进声纹模板（旧 0.7 + 新 0.3），越用越准
...
| 滚轮 | 缩放（0.5~3 倍，自动记住） |
| 右键宠物 | 菜单：连续对话 / 换模型 / 置顶 / 摸头 / 缩放 / 停止播报 / 退出（4 秒自动关闭） |
| 托盘菜单 | 显示/隐藏、**连续对话开关**、换模型、开机自启动、退出 |
| 播报中说话 | 打断并继续听新问题（全双工） |
...
> 7861 给手机浏览器（HTTPS 入口），7860 给安卓遥控 App（它直连主服务，明文 ws），
> 7863 给 App 的「自动发现服务器」。只开 7861 的话浏览器能用，但 App 会一直卡在「正在连接」。
...
**开 App 就能语音使唤，不用开浏览器**：说「小音」→「放首晴天」，**手机的 QQ 音乐**自动播放；
「暂停」「下一首」也控手机上的音乐，回复语音直接在手机上播。**锁屏也能继续听**（浏览器的麦克风一锁屏就被挂起）。
App 里有**对话记录**，你说的话和小音的回答都留在屏幕上，不用开电脑也知道它听清了没有。
需要装一个配套的安卓 App（`android/`，构建与安装见 [android/README.md](android/README.md)）。

为什么不能只用网页：浏览器禁止网页在没有点击的情况下调起别的 App（平台安全设计），
而原生 App 用代码调起不需要确认——所以「说完话就播」只能靠 App 实现。

**电脑端行为完全不变**：服务器按「这次请求从哪来」决定在哪播——

| 你从哪说话 | 歌在哪播 |
|---|---|
| 手机浏览器（页面自动声明为手机端） | 手机（App 没连上时回落到电脑，并提示） |
| 桌宠 / CLI / 电脑网页 | 电脑（与以前完全一致） |

实现上：服务器多了一个 `/ws/device` 端点给 App 常驻连接，音乐工具按来源判定后
把 `{"type":"play","songmid":...}` 推过去；暂停/切歌则走 Android 的媒体会话控制。
App 里点「🔍 自动发现」还能在局域网里自动找到本机（`discovery.py` 应答 UDP 探测），
电脑换网、IP 变了也不用改配置。
...
| `{"type":"continuous","enabled":true}` | 连续对话开关回执（客户端发 `{"type":"continuous","enabled":bool}` 切换） |
| `{"type":"error","error":"..."}` | 错误 |
...
IDLE(KWS检测) ──「小音」──► 发 wake ──► LISTENING(VAD采集) ──► transcript ──► REPLYING
  ...
  └── 开了连续对话时：正常回复完(turn_end) → 继续 LISTENING；静默 N 秒 → 回 IDLE

续听判定（`should_follow_up`，纯函数有单测）：被打断必续听（既有行为）；正常回复完只在
开了连续对话时续听；没听到声音/语音太短/识别为空一律退出，防止环境噪声无限刷轮次。
```
...
- **「我喜欢」歌单**（说「放我喜欢的歌」）同样走界面自动化：点侧栏「喜欢」→ 点页面上的「播放」，多两步，坐标也是比例常量（`_FAVORITE_*_RATIO`）。客户端没提供"放我的收藏"这类接口——桌面端不认 `qqmusic://` 协议（试过，不唤起），本地端口也不是可用 API，所以只剩界面点击这一条路。

已知限制：会员/无版权歌曲客户端自己会提示；点歌和「我喜欢」都要短暂抢一次前台焦点，
**你正在用别的窗口时它会拒绝操作**（每次都重新确认前台还是 QQ 音乐；宁可这一下不动，
也不能把键鼠打进别人的窗口里——这条是真踩过的，不是理论担忧）。
...
├── music.py            # QQ 音乐控制（COM 播放控制 + 界面自动化点歌 + 推给手机 App）
├── devices.py          # 设备推送注册表（手机遥控 App 的在线连接 + 本轮请求来源）
├── discovery.py        # 局域网自动发现（手机 App 广播探测就能找到本机，不用手输 IP）
├── android/            # 小音遥控：安卓 App（手机上播放 QQ 音乐）
...
| CONTINUOUS_TIMEOUT | 10 | 连续对话：回复完后等下一句的秒数，超时休眠 |
| PORT | 7860 | Web 界面端口 |
...
| `test_llm_history.py` | 上下文配平：被截断的工具调用组不能留给 API 畸形历史 | 否 |
| `test_ondevice_asr.py` | **端侧模型选型**：同一批句子对比 Whisper 与端侧小模型的命中率 | 要（edge-tts） |

改识别相关代码（提示词、模型档位、音频处理）后建议跑一遍 `test_stt_accuracy.py` ——
准确率退化是单测覆盖不到的，只有真实音频能暴露。

出问题先看 `xiaoyin.log`（`python app.py` 时自动生成，含完整异常堆栈）。
以前异常被 `except` 直接吞掉，界面上只剩一句「LLM 调用失败」，查不出原因。
...
- [x] 连续对话模式（唤醒一次连着问，超时自动休眠；网页/桌宠/CLI 三端开关）
- [x] 声纹锁定（首次唤醒录入，之后只认主人的声音；高分通过会自适应更新模板）
- [x] 手机端可用（HTTPS 反代 + PWA + 移动音频适配）
- [x] 手机遥控 App：手机上语音点歌，歌从手机 QQ 音乐播放（android/）
- [x] 局域网自动发现（UDP 广播，电脑换网/IP 变了 App 自己找到，不用改配置）
- [x] 「我喜欢」歌单播放（电脑端点侧栏「喜欢」→「播放」；手机端方案在验证）
- [x] 稳定性与可观测性：修掉三个"卡死"级问题（上下文畸形导致 LLM 永久 400、
      配音偶发卡十几秒、声纹比对没归一化导致阈值失效），异常统一写进 `xiaoyin.log`
